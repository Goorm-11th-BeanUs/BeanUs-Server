...
async def coffee_collect(cafe_id: int) -> dict:
    CoffeeHistory01 = CoffeeHistory(history_id=1, cafe_id=cafe_id, request_datetime=datetime.now(tz=timezone(timedelta(hours=9))), amount=500)
    CoffeeHistory02 = CoffeeHistory(history_id=2, cafe_id=cafe_id, request_datetime=datetime.now(tz=timezone(timedelta(hours=9))), amount=1000)
